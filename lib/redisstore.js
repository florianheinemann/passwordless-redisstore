...
// this value can be overridden with a larger integer on options.bcryptRounds
...
RedisStore.prototype.storeOrUpdate = function(token, uid, msToLive, originUrl, callback) {
	...
	self._select(function(err) {
		...
		var runs = self.options.bcryptRounds || bcryptMinRounds;
		if (typeof runs != 'number' || runs < bcryptMinRounds) {
			throw new Error("Invalid bcryptRounds value. Should be an integer greater than "+bcryptMinRounds+".");
		}
