...
// a minimum number of runs for bcrypt is configured below
// this value can be overridden with a larger integer on options.bcryptRuns
...
RedisStore.prototype.storeOrUpdate = function(token, uid, msToLive, originUrl, callback) {
	...
	self._select(function(err) {
		...
		var runs = self.options.bcryptRuns || bcryptMinRuns;
		if (typeof runs != 'number' || runs < bcryptMinRuns) {
			throw new Error("Invalid bcryptRuns value. Should be an integer greater than "+bcryptMinRuns+".");
		}

		bcrypt.hash(token, parseInt(runs, 10), function(err, hashedToken) {
			if(err) {
				return callback(err);
			}
