...
* **[redisstore.rounds]:** *(Number)* Optional. Number of bccrypt rounds to be used. Defaults to: 10

## Hash and salt
As the tokens are equivalent to passwords (even though only for a limited time) they have to be protected in the same way. passwordless-redisstore uses [bcrypt](https://github.com/ncb000gt/node.bcrypt.js/) with automatically created random salts. To generate the salt 10 rounds are used. This number can be overridden with a larger value in the options.
